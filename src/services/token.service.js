...
const generateAuthToken = (userId) => {
  const payload = {
    type: "AUTH_TOKEN",
    data: Buffer.from(userId).toString("base64"),
  };
...
const validateAuthToken = async (token) => {
  try {
    ...
    if (decoded && decoded.type === "AUTH_TOKEN") {
      ...
      return { valid: true, user, expired: false };
    }
  } catch (err) {
...
const generateResetPasswordToken = async (user) => {
  ...
  const payload = {
    ...
    data: Buffer.from(resetPasswordToken.id).toString("base64"),
  };
...
const validateResetPasswordToken = async (token) => {
  try {
    ...
    if (decoded && decoded.type === "RESET_PASSWORD_TOKEN") {
      const id = Buffer.from(decoded.data, "base64").toString();
      const _token = await Token.findById(id);
...
const generateInvitationToken = async (user) => {
  ...
  const payload = {
    ...
    data: Buffer.from(invitationToken.id).toString("base64"),
  };
